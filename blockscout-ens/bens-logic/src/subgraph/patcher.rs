...
impl SubgraphPatcher {
    ...
    pub fn patched_domain(
        ...
        if from_db.name.as_deref() != Some(from_user.inner.name())
            && from_db.id == from_user.inner.id()
        {
            tracing::warn!(
            ...
            from_db.name = Some(from_user.inner.name().to_string());
    ...
    pub fn patched_detailed_domain(
            ...
            && from_db.id == from_user.inner.id()
        {
            ...
            from_db.name = Some(from_user.inner.name().to_string());
            from_db.label_name = Some(from_user.inner.label_name().to_string());
            update_domain_name_in_background(pool, from_user.clone());
        };
...
fn update_domain_name_in_background(pool: Arc<PgPool>, domain_name: DomainNameOnProtocol) {
    ...
    tokio::spawn(async move {
        let name = domain_name.name().to_string();
