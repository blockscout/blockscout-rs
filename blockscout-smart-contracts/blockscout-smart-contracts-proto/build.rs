...
fn compile(
    ...
    let swagger_filename = "blockscout-smart-contracts";
    ...
    config
        ...
        for default_field in default_fields {
            config.field_attribute(
                format!(".blockscout.blockscout-smart-contracts.v1.{default_field}"),
                "#[serde(default)]",
            );
        }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile(
        &["proto/v1/blockscout_smart_contracts.proto", "proto/v1/health.proto"],
        &["proto", "../../proto"],
        gens,
    )?;
    Ok(())
