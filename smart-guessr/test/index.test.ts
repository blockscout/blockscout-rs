...
describe('Processing abi requests', () => {
    ...
    // `sigAlts` carries alternative signatures for a selector, resolved through
    // public signature databases that accumulate collision spam over time. It
    // drifts independently of this service, so assert on the resolved ABI only.
    function withoutSigAlts(abi: any) {
        return abi.map(({ sigAlts, ...rest }: any) => rest);
    }
    ...
    it('returns an abi with blockscout as provider', async () => {
        ...
        expect(withoutSigAlts(await response.json())).toEqual(expected);
    })
