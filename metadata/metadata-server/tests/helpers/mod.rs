...
pub async fn init_metadata_server<F>(db_url: String, settings_setup: F) -> Url
where
    F: Fn(Settings) -> Settings,
{
    ...
    test_server::init_server(|| metadata_server::run(settings), &base).await;
    base
}
