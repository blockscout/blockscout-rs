use basic_cache_server::Settings;
...
pub async fn init_basic_cache_server<F>(db_url: String, settings_setup: F) -> Url
where
    F: Fn(Settings) -> Settings,
{
    ...
    test_server::init_server(|| basic_cache_server::run(settings), &base).await;
    base
}
