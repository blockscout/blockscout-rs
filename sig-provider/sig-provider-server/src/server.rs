use crate::{settings::SourcesSettings, Service, Settings};
...
pub fn http_server<S: SignatureService, A: AbiService>(
    ...
    let server = HttpServer::new(move || {
        App::new().configure(|config| http_configure(config, signature.clone(), abi.clone()))
    })
    .bind(addr)
    ...
    if settings.server.http.enabled {
        let http_server = {
            let http_server_future =
                http_server(service.clone(), service.clone(), settings.server.http.addr);
