use super::{bytecodes_comparison::extract_constructor_args, candidates::BytecodeCandidate};
use crate::search::candidates::find_bytecode_candidates;
use bytes::Bytes;
use entity::{sea_orm_active_enums::BytecodeType, sources};
use ethabi::Constructor;
use sea_orm::{entity::prelude::*, ConnectionTrait};

pub struct BytecodeRemote {
    pub bytecode_type: BytecodeType,
    pub data: Bytes,
}

#[derive(Debug)]
pub struct PartialMatchContract {
    pub source: sources::Model,
    pub constructor_args: Option<Bytes>,
}

pub async fn find_partial_match_contract<C>(
    db: &C,
    remote: BytecodeRemote,
) -> Result<Option<PartialMatchContract>, anyhow::Error>
where
    C: ConnectionTrait,
{
    let candidates = {
        let now = std::time::Instant::now();
        let candidates = find_bytecode_candidates(db, &remote).await?;
        tracing::debug!(
            candidates_len = candidates.len(),
            elapsed = now.elapsed().as_secs_f64(),
            "finished bytecode candidates search",
        );
        candidates
    };
    let found_bytecode = candidates
        .into_iter()
        .find(|c| c.is_match(&remote.data).is_ok());
    let found_bytecode = match found_bytecode {
        Some(found_bytecode) => found_bytecode,
        None => return Ok(None),
    };
    tracing::debug!(id = found_bytecode.bytecode.id, "found 1 bytecode");
    let contract = get_contract_by_bytecode(db, found_bytecode, remote).await?;
    Ok(Some(contract))
}

async fn get_contract_by_bytecode<C>(
    db: &C,
    bytecode: BytecodeCandidate,
    remote: BytecodeRemote,
) -> Result<PartialMatchContract, anyhow::Error>
where
    C: ConnectionTrait,
{
    let source = sources::Entity::find_by_id(bytecode.bytecode.source_id)
        .one(db)
        .await?;
    let source = source
        .ok_or_else(|| DbErr::RecordNotFound("bytecode doesn't have valid source_id".into()))?;

    let constructor = get_constructor(source.abi.clone())?;
    let is_creation_input = remote.bytecode_type == BytecodeType::CreationInput;
    let constructor_args = extract_constructor_args(
        &remote.data,
        &bytecode.raw(),
        constructor.as_ref(),
        is_creation_input,
    )?;
    Ok(PartialMatchContract {
        source,
        constructor_args,
    })
}

fn get_constructor(
    abi: Option<serde_json::Value>,
) -> Result<Option<Constructor>, serde_json::Error> {
    match abi {
        Some(abi) => {
            let abi: ethers_core::abi::Abi = serde_json::from_value(abi)?;
            Ok(abi.constructor)
        }
        None => Ok(None),
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::DisplayBytes;
    use entity::{bytecodes, parts, sea_orm_active_enums::PartType};
    use pretty_assertions::assert_eq;
    use sea_orm::{DatabaseBackend, MockDatabase};
    use std::str::FromStr;

    /// Contract code:
    /// ```solidity
    /// pragma solidity ^0.8.7;
    /// contract Number {
    ///     uint public number;
    ///     string public str;
    ///     
    ///     constructor(uint _number, string memory _str) {
    ///         number = _number;
    ///         str = _str;
    ///     }
    /// }
    /// ```
    const NUMBER_MAIN_PART: &str = "";
    const NUMBER_META_PART: &str = "a26469706673582212202ec25b2395cacdfaf72db8374301e337eda2a878ca3089a34d47f0cf8d2968fc64736f6c63430008110033";
    const NUMBER_ARGS_PART: &str = "";

    fn source() -> sources::Model {
        sources::Model {
            id: 1,
            source_type: entity::sea_orm_active_enums::SourceType::Solidity,
            compiler_version: "v0.8.7".into(),
            compiler_settings: serde_json::json!({"settings": true}),
            file_name: "Number.sol".into(),
            contract_name: "Number".into(),
            abi: Some(
                serde_json::json!([ { "inputs": [ { "internalType": "uint256", "name": "_number", "type": "uint256" } ], "stateMutability": "nonpayable", "type": "constructor" }, { "inputs": [], "name": "number", "outputs": [ { "internalType": "uint256", "name": "", "type": "uint256" } ], "stateMutability": "view", "type": "function" } ]),
            ),
            raw_creation_input: vec![],
            raw_deployed_bytecode: vec![],
            created_at: Default::default(),
            updated_at: Default::default(),
        }
    }

    fn get_parts(bytecodes: &[&str]) -> Vec<parts::Model> {
        bytecodes
            .iter()
            .enumerate()
            .map(|(i, bytecode)| {
                let part_type = if i % 2 == 0 {
                    PartType::Main
                } else {
                    PartType::Metadata
                };
                parts::Model {
                    id: i as i64,
                    part_type,
                    data: hex::decode(bytecode).unwrap(),
                    created_at: Default::default(),
                    updated_at: Default::default(),
                }
            })
            .collect()
    }

    fn candidate(parts: &[&str], source_id: i64, ty: BytecodeType) -> BytecodeCandidate {
        BytecodeCandidate {
            parts: get_parts(parts),
            bytecode: bytecodes::Model {
                id: 1,
                created_at: Default::default(),
                updated_at: Default::default(),
                source_id,
                bytecode_type: ty,
            },
        }
    }

    #[tokio::test]
    async fn test_get_contract() {
        let source = source();
        let db = MockDatabase::new(DatabaseBackend::Postgres)
            .append_query_results(vec![vec![source.clone()]])
            .into_connection();

        let bytecode = candidate(
            &[NUMBER_MAIN_PART, NUMBER_META_PART],
            1,
            BytecodeType::CreationInput,
        );

        let remote = BytecodeRemote {
            bytecode_type: BytecodeType::CreationInput,
            data: DisplayBytes::from_str(
                &vec![NUMBER_MAIN_PART, NUMBER_META_PART, NUMBER_ARGS_PART].join(""),
            )
            .unwrap()
            .0,
        };
        let result = get_contract_by_bytecode(&db, bytecode, remote)
            .await
            .expect("unexpected error");

        assert_eq!(
            hex::encode(result.constructor_args.expect("args shoud be Some")),
            NUMBER_ARGS_PART,
        );
        assert_eq!(result.source, source,);
    }

    #[tokio::test]
    async fn test_get_contract_failed() {
        let invalid_args = "6080609001fe";

        let source = source();
        let db = MockDatabase::new(DatabaseBackend::Postgres)
            .append_query_results(vec![vec![source.clone()]])
            .into_connection();

        let bytecode = candidate(
            &[NUMBER_MAIN_PART, NUMBER_META_PART],
            1,
            BytecodeType::CreationInput,
        );

        let remote = BytecodeRemote {
            bytecode_type: BytecodeType::CreationInput,
            data: DisplayBytes::from_str(
                &vec![NUMBER_MAIN_PART, NUMBER_META_PART, invalid_args].join(""),
            )
            .unwrap()
            .0,
        };
        let _ = get_contract_by_bytecode(&db, bytecode, remote)
            .await
            .expect_err("expected error during decoding constructor arguments");
    }
}
