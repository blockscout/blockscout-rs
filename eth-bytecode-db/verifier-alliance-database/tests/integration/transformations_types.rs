...
impl TestCase {
    pub fn from_path(test_case_path: &PathBuf) -> Self {
        let content =
            std::fs::read_to_string(test_case_path).expect("failed to read test case data");
...
impl TestCase {
    ...
    async fn validate_code_value(
        ...
        let code_model = code::Entity::find()
            ...
            .expect(&format!(
                "code hash does not exist in the database: {:?}",
                code_hash
            ));
    ...
    async fn validate_sources_table(
        ...
        for (path, content) in self.sources.iter() {
            let source = sources
                ...
                .expect(&format!("source not found in the database for path={path}"));
    ...
    async fn validate_compiled_contracts_sources_table(
        ...
        for (path, content) in self.sources.iter() {
            let compiled_contract_source = compiled_contracts_sources
                ...
                .expect(&format!(
                    "compiled contract source not found in database for path={path}"
                ));
            let source = sources
                .iter()
                .find(|source| &source.content == content)
