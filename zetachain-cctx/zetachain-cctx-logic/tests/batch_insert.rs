...
let cctx = cctx.unwrap();
 ...
 let token_id = token::Entity::find().filter(token::Column::Asset.eq(token.asset)).one(db.client().as_ref()).await.unwrap().unwrap().id;

 assert_eq!(cctx.token_id, Some(token_id));
