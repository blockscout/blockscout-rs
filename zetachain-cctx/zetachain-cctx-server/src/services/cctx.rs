...
use zetachain_cctx_logic::models::CompleteCctx;
...
pub fn transform_complete_cctx_to_cross_chain_tx(entity: CompleteCctx) -> Result<CrossChainTx, String> {
    let cctx = CrossChainTx {
        ...
        .map(|r|  -> Result<RelatedCctx, String> {
            Ok(RelatedCctx {
                ...
                .ok_or(format!("RelatedCctx: Index: {}, Invalid status: {}", &r.index, r.status))?.into(),
                inbound_amount: r.inbound_amount,
                inbound_coin_type: CoinType::from_str_name(r.inbound_coin_type.as_str())
                .ok_or(format!("RelatedCctx: Index: {}, Invalid coin type: {}", &r.index, r.inbound_coin_type))?.into(),
                outbound_params: r.outbound_params.into_iter().map(|o| {Ok(RelatedOutboundParams {
                    ...
                    .ok_or(format!("RelatedOutbound: Index: {}, Invalid coin type: {}", &r.index, o.coin_type))?.into(),
...
impl CctxInfo for CctxService {
    ...
    async fn get_cctx_info(&self, request: Request<GetCctxInfoRequest>) -> Result<Response<CrossChainTx>, Status> {
        ...
        let cctx = transform_complete_cctx_to_cross_chain_tx(entity).map_err(|e| Status::internal(e))?;
