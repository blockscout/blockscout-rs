...
class CronVisualizerGUI:
    def __init__(self, root):
        ...
        self.use_config_durations = tk.BooleanVar(
            value=False
        )  # Toggle for duration source
    ...
    def setup_gui(self):
        ...
        duration_entry = ttk.Entry(
            radiobutton_frame_1, textvariable=self.manual_duration_var, width=5
        )
        ...
        self.schedule_list = ttk.Treeview(
            list_frame, columns=("Schedule", "Times"), show="headings"
        )
    ...
    def parse_cron_schedule(
        self, schedule: str, target_date: datetime
    ) -> List[datetime]:
    ...
    def get_task_overlaps(self) -> List[List[str]]:
        ...
        for name, schedule in self.schedules.items():
            ...
            for start_time in start_times:
                ...
                for minute in range(
                    start_minute, min(start_minute + duration, 24 * 60)
                ):
        ...
        if 20 <= y <= self.canvas_height - 40:
            ...
            if 0 <= minute_index < 24 * 60:
                ...
                items = self.canvas.find_overlapping(
                    x - 1, 20, x + 1, self.canvas_height - 40
                )
                if items:
                    for item in items:
                        ...
                        tasks_tag = next(
                            (tag for tag in tags if tag.startswith("tasks_")), None
                        )
                        if tasks_tag:
                            tasks = tasks_tag[6:].split(
                                "/"
                            )  # Fix 2: Change separator to '/'
                            count = len(tasks)
