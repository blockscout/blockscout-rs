...
| `STATS__CONDITIONAL_START__INTERCHAIN_CATCHUP_MIN_PROGRESS__ENABLED` |                          | Interchain mode only, off by default. When enabled, the service waits before its first chart update until every `(bridge, chain)` pair relevant to `interchain_filter` reports at least the configured catch-up progress. **Requires `STATS__INTERCHAIN_INDEXER_API_URL`** — enabling this without it is a startup error. Note that setting only `…__THRESHOLD` also enables this check. | `true`                                          |
| `STATS__CONDITIONAL_START__INTERCHAIN_CATCHUP_MIN_PROGRESS__THRESHOLD` |                          | Minimum interchain catch-up progress required before the first chart update, as a ratio (`0.95` = 95%). This is the **scanned** share of the configured block range, never a completeness measure — a range scanned but failed downstream still counts. Aggregated as the minimum over the relevant pairs. Gates the **first update only**; the per-cycle catch-up check (`STATS__INTERCHAIN_INDEXER_API_URL`) governs rebuilds after the start. Setting this alone also enables the check. | `0.98`                                          |
