...
| `STATS__INTERCHAIN_INDEXER_API_URL` |                          | Base URL of the interchain indexer's HTTP API, used in `Interchain` mode to read `GET /api/v1/status/indexing`. When a `(bridge, chain)` pair relevant to the configured interchain filter is still catching up, every interchain chart is recomputed from the indexer's current earliest data instead of only moving forward, so history the indexer backfills is picked up. Optional: with no URL the check is disabled and charts still pick up history that extends backwards, but not interior gaps. Ignored outside `Interchain` mode. | `null`                                          |
...
| `STATS__CONDITIONAL_START__ZETACHAIN_INDEXED_UNTIL_TODAY__ENABLED`     |                          | Enable checking zetachain indexing stats, e.g. `false`. If not set, value is the same as `STATS__MODE=zetachain`                                                                                                                                                                                                                                                                                                                                              | `null`                                          |
| `STATS__CONDITIONAL_START__INTERCHAIN_CATCHUP_MIN_PROGRESS__ENABLED` |                          | **Disabled (`false`) by default** — ignore the Default column to the right for this row; it reads `true` due to a known limitation in the docs generator for a field whose own default diverges from its type's built-in default (see `.memory-bank/gotchas.md`). Interchain mode only. When enabled, the service waits before its first chart update until every `(bridge, chain)` pair relevant to `interchain_filter` reports at least the configured catch-up progress. **Requires `STATS__INTERCHAIN_INDEXER_API_URL`** — enabling this without it is a startup error. Note that setting only `…__THRESHOLD` also enables this check. | `true`                                          |
| `STATS__CONDITIONAL_START__INTERCHAIN_CATCHUP_MIN_PROGRESS__THRESHOLD` |                          | Only takes effect once the gate above is enabled — **disabled by default**, so this value gates nothing out of the box despite the `0.98` shown to the right. Minimum interchain catch-up progress required before the first chart update, as a ratio (`0.95` = 95%). This is the **scanned** share of the configured block range, never a completeness measure — a range scanned but failed downstream still counts. Aggregated as the minimum over the relevant pairs. Gates the **first update only**; the per-cycle catch-up check (`STATS__INTERCHAIN_INDEXER_API_URL`) governs rebuilds after the start. Setting this alone also enables the check. | `0.98`                                          |
...
| `STATS__ENABLE_ALL_FILECOIN`                                           |                          | Enable the Filecoin-specific API surface: `filecoinChainFeesGrowth` is enabled under its own id; the public `txnsFee` id is force-enabled and served with the `filecoinNewChainFees` implementation (chain-wide fees), and the public `txnsFee24h` id is force-enabled and served with the `filecoinChainFees24h` implementation (24h burn + tips); under this flag, `filecoinNewChainFees` and `filecoinChainFees24h` are never exposed as public chart ids. | `false`                                         |
| `STATS__API_KEYS__<KEY_NAME>`                                          |                          | E.g. `very_secure_key_value`. Allows access to key-protected functionality                                                                                                                                                                                                                                                                                                                                                                                    | `null`                                          |
...
##### Conditional start
In order to prevent incorrect statistics from being collected, there is an option to automatically delay chart update. This is controlled by `STATS__CONDITIONAL_START__*` environmental variables. 
...
| `STATS_CHARTS__LINE_CHARTS__<LINE_CHART_NAME>__RESOLUTIONS__DAY`   |                          | Enable daily data for the chart, e.g. `true`                                                                                                                                                   | `null`        |
| `STATS_CHARTS__LINE_CHARTS__<LINE_CHART_NAME>__RESOLUTIONS__WEEK`  |                          | Enable weekly data                                                                                                                                                                             | `null`        |
| `STATS_CHARTS__LINE_CHARTS__<LINE_CHART_NAME>__RESOLUTIONS__MONTH` |                          | Enable monthly data                                                                                                                                                                            | `null`        |
| `STATS_CHARTS__LINE_CHARTS__<LINE_CHART_NAME>__RESOLUTIONS__YEAR`  |                          | Enable yearly data                                                                                                                                                                             | `null`        |
