...
| `STATS__MODE`                                                        |                          | Service mode: `blockscout`, `aggregator`, `zetachain`, or `interchain`. Modes are mutually exclusive. `blockscout` implies no zetachain CCTX and no multichain behavior.                                                        | `blockscout`                                    |
| `STATS__MULTICHAIN_FILTER`                                           |                          | An array of chain IDs used to filter input data in `aggregator` mode. Specified as a comma-separated list of identifiers without spaces (e.g. `10,130`)                                                                              | `null`                                          |
