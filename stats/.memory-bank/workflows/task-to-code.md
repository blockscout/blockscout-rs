...
- After the latest code edit, run `just format`, then `graphify update .` to keep the local code graph current, before final validation and reporting. If either cannot be run, say exactly why.
...
`just format` applies `cargo sort --workspace` (if installed) and
`cargo fmt --all -- --config imports_granularity=Crate` — see
`.memory-bank/rules/rust-style.md`.

`graphify update .` is AST-only and costs no API calls. The graph lives at
`graphify-out/`, which is gitignored (`stats/.gitignore`) and never
committed — so keeping it fresh costs nothing in diff. Skipping it does not
create risk for other clones of the repo, but it does leave *this* session's
(and the next session's) local navigation stale against the code just
changed, which can mislead later query/path/explain results. If `graphify`
is not installed, or no graph exists yet at `graphify-out/graph.json` and it
cannot be bootstrapped in this environment, say so explicitly instead of
silently skipping the step — see `.memory-bank/rules/graphify.mdc`.
