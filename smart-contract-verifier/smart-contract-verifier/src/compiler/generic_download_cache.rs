use super::generic_fetcher::{FetchError, Fetcher, Version};
use crate::metrics;
use std::{collections::HashMap, path::PathBuf, str::FromStr, sync::Arc};
use tracing::Instrument;

#[derive(Default)]
pub struct DownloadCache<Ver> {
    cache: parking_lot::Mutex<HashMap<Ver, Arc<tokio::sync::RwLock<Option<PathBuf>>>>>,
}
...
mod tests {
    use super::{
        super::{generic_list_fetcher::ListFetcher, version as evm_version},
        *,
    };
    ...
    fn new_version(major: u64) -> evm_version::Version {
        evm_version::Version::Release(evm_version::ReleaseVersion {
    ...
    fn value_is_cached() {
        #[derive(Default)]
        struct MockFetcher {
            counter: parking_lot::Mutex<HashMap<evm_version::Version, u32>>,
        }

        #[async_trait]
        impl Fetcher for MockFetcher {
            type Version = evm_version::Version;
        ...
        let get_and_check = |ver: &evm_version::Version| {
    ...
    async fn filter_versions() {
        let versions: HashSet<evm_version::Version> =
    ...
    async fn load_downloaded_compiler() {
        let ver = evm_version::Version::from_str("0.7.0+commit.9e61f92b").unwrap();
        let dir = temp_dir();
