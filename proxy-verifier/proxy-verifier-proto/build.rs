...
fn compile(
    ...
    config
        ...
        .protoc_arg("grpc_api_configuration=proto/v1/api_config_http.yaml,output_format=yaml,allow_merge=true,merge_file_name=proxy-verifier")
        .bytes(["."])
        .btree_map(["."])
        .type_attribute(".", "#[actix_prost_macros::serde]")
        // .field_attribute(
        //     ".blockscout.proxyVerifier.v1.<MessageName>.<DefaultFieldName>",
        //     "#[serde(default)]"
        // )
        ;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile(
        &["proto/v1/proxy-verifier.proto", "proto/v1/health.proto"],
        &["proto/v1"],
        gens,
    )?;
    Ok(())
