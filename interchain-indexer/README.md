...
| `INTERCHAIN_INDEXER__STATS__INCLUDE_ZERO_CHAINS`                        |                          | When `true`, stats endpoints include known chains from `chains` even when the aggregated stats row is missing or has a zero value. Disable it to return only chains with positive aggregated stats. | `true`        |
