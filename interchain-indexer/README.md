...
| `INTERCHAIN_INDEXER__AVALANCHE_INDEXER__FAILURE_RETRY__BACKOFF_BASE`                 |                          | Base delay used for initial due calculation and between complete retry sweeps. An active sweep continues across ticks without this delay. Unit: `seconds`                                                                                                                                                                                                                               | `30`          |
| `INTERCHAIN_INDEXER__AVALANCHE_INDEXER__FAILURE_RETRY__BACKOFF_CAP`                  |                          | Ceiling of the capped exponential backoff between wholly unsuccessful sweeps. This makes singleton retry forever affordable. Unit: `seconds`                                                                                                                                                                                                                                           | `3600`        |
...
| `INTERCHAIN_INDEXER__AVALANCHE_INDEXER__FAILURE_RETRY__MAX_CHUNKS_PER_PASS`          |                          | Maximum number of adaptively sized chunks emitted per retry tick, across all ready intervals and chains. Each chunk is at most `batch_size` blocks; the bridge-wide budget bounds replay RPC load.                                                                                                                                                                                       | `8`           |
...
| `INTERCHAIN_INDEXER__AVALANCHE_INDEXER__FAILURE_RETRY__SCAN_INTERVAL`                |                          | How often the retry tick reads the full open-ledger snapshot. Unit: `seconds`                                                                                                                                                                                                                                                                                                            | `60`          |
| `INTERCHAIN_INDEXER__AVALANCHE_INDEXER__FAILURE_RETRY__SPLIT_AFTER_ATTEMPTS`         |                          | Initial ledger record plus wholly unsuccessful replay sweeps required before request width starts halving. Width changes only after a complete sweep, never between its chunks.                                                                                                                                                                                                         | `3`           |
...
| `INTERCHAIN_INDEXER__AMB_INDEXER__FAILURE_RETRY__BACKOFF_BASE`                 |                          | Base delay used for initial due calculation and between complete retry sweeps. An active sweep continues across ticks without this delay. Unit: `seconds`                                                                                                                                                                                                                               | `30`          |
| `INTERCHAIN_INDEXER__AMB_INDEXER__FAILURE_RETRY__BACKOFF_CAP`                  |                          | Ceiling of the capped exponential backoff between wholly unsuccessful sweeps. This makes singleton retry forever affordable. Unit: `seconds`                                                                                                                                                                                                                                            | `3600`        |
...
| `INTERCHAIN_INDEXER__AMB_INDEXER__FAILURE_RETRY__MAX_CHUNKS_PER_PASS`          |                          | Maximum number of adaptively sized chunks emitted per retry tick, across all ready intervals and chains. Each chunk is at most `batch_size` blocks; the bridge-wide budget bounds replay RPC load.                                                                                                                                                                                      | `8`           |
...
| `INTERCHAIN_INDEXER__AMB_INDEXER__FAILURE_RETRY__SCAN_INTERVAL`                |                          | How often the retry tick reads the full open-ledger snapshot. Unit: `seconds`                                                                                                                                                                                                                                                                                                           | `60`          |
| `INTERCHAIN_INDEXER__AMB_INDEXER__FAILURE_RETRY__SPLIT_AFTER_ATTEMPTS`         |                          | Initial ledger record plus wholly unsuccessful replay sweeps required before request width starts halving. Width changes only after a complete sweep, never between its chunks.                                                                                                                                                                                                         | `3`           |

[anchor]: <> (anchors.envs.end.amb)

### xDai Indexer Settings

[anchor]: <> (anchors.envs.start.xdai)

| Variable                                                                             | Req&#x200B;uir&#x200B;ed | Description                                                                                                                                                                                                                                                                                                                                                                             | Default value |
| ------------------------------------------------------------------------------------ | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__BATCH_SIZE`                                  |                          | Number of blocks scanned per log request.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `1000`        |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__PULL_INTERVAL_MS`                            |                          | Duration between pulling xDai bridge events. Unit: `milliseconds`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `500`         |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__RECEIPT_CONCURRENCY`                         |                          | Maximum concurrent receipt and block fetches per xDai batch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `25`          |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__FAILURE_RETRY__BACKOFF_BASE`                 |                          | Base delay of the capped exponential backoff between replay attempts for a recorded failed block range. Unit: `seconds`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `30`          |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__FAILURE_RETRY__BACKOFF_CAP`                  |                          | Ceiling of the capped exponential backoff between replay attempts. This is the knob that makes retrying a permanently unrecoverable interval forever affordable. Unit: `seconds`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `3600`        |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__FAILURE_RETRY__ENABLED`                      |                          | Kill switch for the failed-range replay pass. Recording failed ranges still happens when `false`; only replay of already-recorded ranges is paused.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `true`        |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__FAILURE_RETRY__MAX_CHUNKS_PER_PASS`          |                          | Maximum number of `batch_size`-sized chunks replayed per retry tick, across all due failed ranges on every chain of the bridge. The pass runs as a sibling future of the per-chain handlers, so it no longer holds up the catch-up and realtime scans; the budget now bounds replay RPC load per pass against the same rate-limited endpoints. Keeping it low only starves the backlog.                                                                                                                                                                                                                                                                                                                                                                                          | `8`           |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__FAILURE_RETRY__RECORD_RETRY_ATTEMPTS`        |                          | Number of attempts to persist a failed range to `indexer_failures` before the indexer gives up and stops (enters a failed state).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `3`           |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__FAILURE_RETRY__RECORD_RETRY_INITIAL_BACKOFF` |                          | Initial delay before retrying a failed `indexer_failures` write; doubles on each subsequent retry. Unit: `milliseconds`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `200`         |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__FAILURE_RETRY__SCAN_INTERVAL`                |                          | How often the retry pass scans recorded failed ranges for ones that are due for another replay attempt. Unit: `seconds`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `60`          |
| `INTERCHAIN_INDEXER__XDAI_INDEXER__FAILURE_RETRY__SPLIT_AFTER_ATTEMPTS`         |                          | Initial ledger record plus wholly unsuccessful replay sweeps required before request width starts halving. Width changes only after a complete sweep, never between its chunks.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `3`           |

[anchor]: <> (anchors.envs.end.xdai)
